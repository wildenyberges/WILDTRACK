// wildtrack-alerts.js v3
// Webhook para alertas de Traccar → AWS SES
// Lee alertEmail de attributes del usuario en Traccar dinámicamente
...
const CONFIG = {
  ...
  fallbackTo:  'bergro@gpstracking.com.do', // si el usuario no tiene alertEmail
  traccar:     'http://localhost:8082',
  traccarAuth: 'Basic ' + Buffer.from('wildenyberges:T@ngar0ja').toString('base64'),

  // Solo estos tipos generan correo
  allowedTypes: [
...
// ── CACHE DE EMAILS (evita consultas repetidas) ───────────────
const emailCache = {}; // deviceId → { email, ts }
const CACHE_TTL  = 5 * 60 * 1000; // 5 minutos

async function getAlertEmail(deviceId) {
  // Revisar caché
  const cached = emailCache[deviceId];
  if (cached && (Date.now() - cached.ts) < CACHE_TTL) {
    return cached.email;
  }

  try {
    // 1) Obtener permisos del dispositivo → usuarios
    const permRes = await fetch(
      `${CONFIG.traccar}/api/permissions?deviceId=${deviceId}`,
      { headers: { 'Authorization': CONFIG.traccarAuth } }
    );
    const perms = await permRes.json();

    // Buscar el primer usuario no-admin con alertEmail
    for (const perm of perms) {
      if (!perm.userId) continue;
      const userRes = await fetch(
        `${CONFIG.traccar}/api/users/${perm.userId}`,
        { headers: { 'Authorization': CONFIG.traccarAuth } }
      );
      const user = await userRes.json();
      if (user.attributes && user.attributes.alertEmail) {
        const email = user.attributes.alertEmail;
        emailCache[deviceId] = { email, ts: Date.now() };
        return email;
      }
    }
  } catch(e) {
    console.error('[WARN] No se pudo obtener alertEmail:', e.message);
  }

  // Sin email configurado — usar fallback
  emailCache[deviceId] = { email: CONFIG.fallbackTo, ts: Date.now() };
...
<body style="margin:0;padding:0;background:#f1f5f9;font-family:Arial,sans-serif;">
        ...
        <tr>
          <td style="padding:8px 28px 24px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              ...
              ${speedKmh ? `<tr><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;">
              ...
              ${address ? `<tr><td style="padding:8px 0;border-bottom:1px solid #f1f5f9;">
              ...
              ${mapsLink ? `<tr><td style="padding:12px 0 0;">
  ...
  return { subject: `WILDTRACK · ${alert.label} — ${devName}`, html };
}
...
app.post('/webhook', async (req, res) => {
  ...
  const deviceId = payload.event?.deviceId || payload.deviceId;
  const devName  = payload.device?.name || `#${deviceId}`;
  console.log(`[${new Date().toISOString()}] ${type} — ${devName}`);

  const { subject, html } = buildEmail(payload);
  const to = await getAlertEmail(deviceId);
  console.log(`[→] ${to}`);
...
app.get('/test', async (req, res) => {
  const payload = {
    event:    { type: 'ignitionOn', eventTime: new Date().toISOString(), deviceId: 2 },
    device:   { name: 'TOYOTA HILUX ROJO VINO' },
    position: { speed: 45, latitude: 18.4861, longitude: -69.9312, address: 'Av. Winston Churchill, Santo Domingo' }
  };
  const { subject, html } = buildEmail(payload);
  const to = await getAlertEmail(2);
  try {
    await transporter.sendMail({ from: CONFIG.from, to, subject, html });
    res.send(`✅ Correo enviado a ${to}`);
  } catch(e) {
...
app.listen(CONFIG.port, () => {
  console.log(`WILDTRACK Alerts v3 corriendo en puerto ${CONFIG.port}`);
});
